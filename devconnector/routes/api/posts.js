...
router.post(
  '/',
  [auth, [body('text', 'A comment text is required').not().isEmpty()]],
  async (req, res) => {
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    //check if this post is the users post
    ...
    res.send(`Deleted post ${post.text}`);
  } catch (err) {
    console.log(err.message);
    if (error.kind === 'ObjectId')
      return res.status(400).json({ msg: 'No post found' });
...
router.delete('/like/:id', auth, async (req, res) => {
  try {
    ...
    if (index === -1) {
      return res.status(400).json({ msg: 'You must like a post first.' });
    }
